...
ws = WebSocket(
    "wss://stream.bybit.com/realtime", subscriptions=["instrument_info.100ms.BTCUSD"]
)


class HTTPTest(unittest.TestCase):
    def test_orderbook(self):
        self.assertEqual(session.orderbook(symbol="BTCUSD")["ret_msg"], "OK")
    ...
    def test_latest_information_for_symbol(self):
        self.assertEqual(session.latest_information_for_symbol()["ret_msg"], "OK")

    def test_public_trading_records(self):
        self.assertEqual(
            session.public_trading_records(symbol="BTCUSD")["ret_msg"], "OK"
        )

    def test_query_symbol(self):
        self.assertEqual(session.query_symbol()["ret_msg"], "OK")

    def test_server_time(self):
        self.assertEqual(session.server_time()["ret_msg"], "OK")

    def test_announcement(self):
        self.assertEqual(session.announcement()["ret_msg"], "OK")
